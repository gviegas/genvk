...
class Extension:
    ...
    GUARDS = {
        NA: ("", ""),
        COMMON: ("", ""),
        ANDROID: ("#ifdef VK_USE_PLATFORM_ANDROID_KHR\n", "#endif\n"),
    ...
    def __init__(self, command):
        ...
        for e in Extension.NAMES_COMMON:
            if e == command.proto.name:
                ...
                return
        for e in Extension.NAMES_ANDROID:
            if e == command.proto.name:
                self.category = Extension.ANDROID
                return
...
def gen_commands(registry):
    ...
    if cmds is None:
        print("[err] bad xml file: 'commands' element not found")
        exit()
...
def gen_version(registry):
    ...
    if types is None:
        print("[err] bad xml file: 'types' element not found")
        exit()
...
def gen_lib():
    if os.name == "posix":
        ...
        # TODO
        print("[err] gen_lib not yet implemented for " + os.name)
        exit()


def fmt_vk_h():
    fmt = ["clang-format", "-i", "--fallback-style=none"]
    try:
        subprocess.run(fmt + [VK_H], check=True)
    except FileNotFoundError:
        print("[warn] clang-format not found")
...
def gen(xml_path):
    ...
    if root.tag != "registry":
        print("[err] bad xml file: unexpected root element '" + root.tag + "'")
        exit()
    ...
    if oflag != "w":
        ...
        for p in [VK_H, VK_LIB]:
            try:
                f = open(p)
                f.close()
                print("[err] file '{}' exists".format(p))
                exit()
            except FileNotFoundError:
                pass
    gen(pathname)
